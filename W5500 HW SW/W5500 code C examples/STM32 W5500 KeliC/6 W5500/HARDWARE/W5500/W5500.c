...
void W5500_Init(void)
{
	...
	Write_W5500_1Byte(SIMR,S0_IMR|S1_IMR);			//socket中断状态0-7 寄存器配置在S0上
	Write_W5500_SOCK_1Byte(0, Sn_IMR, IMR_SENDOK | IMR_TIMEOUT | IMR_RECV | IMR_DISCON | IMR_CON);	//S0 中断功能全部打开
	Write_W5500_SOCK_1Byte(1, Sn_IMR, IMR_SENDOK | IMR_TIMEOUT | IMR_RECV | IMR_DISCON | IMR_CON);	//S1 中断功能全部打开
}
...
void Socket_Init(SOCKET s)
{
	...
	switch(s)
	{
		...
		case 1:
			//设置端口1的端口号
			Write_W5500_SOCK_2Byte(1, Sn_PORT, S0_Port[0]*256+S0_Port[1]);
			//设置端口1目的(远程)端口号
			Write_W5500_SOCK_2Byte(1, Sn_DPORTR, S0_DPort[0]*256+S0_DPort[1]);
			//设置端口1目的(远程)IP地址
			Write_W5500_SOCK_4Byte(1, Sn_DIPR, S0_DIP);	
...
//	}
	...
	if((i & S1_INT) == S1_INT)//Socket0事件处理 
	{
		...
		Write_W5500_SOCK_1Byte(1,Sn_IR,j);//回写清除中断标志
