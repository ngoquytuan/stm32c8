...
TCP server mode
This program is a server mode extension, currently supports 2 clients in the state of W5500 bit server.
Since there are 8 ports, it supports up to 8 clients.

Note: When the W5500 opens multiple ports on the same server, each port uses the same port number.
?????? This program is a port extension program, and both port 0 and port 1 are opened. If more ports are opened, you can continue to fill in according to the routine.

Interrupt judgment method
The TCP server mode sets the module itself as a primary server, and the other ports connected to it are clients.
Therefore, this mode only needs to determine the module's own IP and port, it can accept access to all network devices under the gateway.


W5500所有引脚对应如下： 
sck PA5  miso PA6  mosi PA7  rst PA2  int PA3  scs PA0
...
int main(void)
   {
 	...
 	while(1)
	{
		...
		{
			S1_Data&=~S_RECEIVE;
			Process_Socket_Data(1);//W5500接收并发送接收到的数据
		}	
		...
		else if(W5500_Send_Delay>=720000)//定时发送字符串
		{
			...
			if(S1_State == (S_INIT|S_CONN))
			{
				...
				memcpy(Tx_Buffer, "\r\n启光科技 W5500服务器TEST\r\n", 30);	
				Write_SOCK_Data_Buffer(1, Tx_Buffer, 30);//指定Socket(0~7)发送数据处理,端口0发送30字节数据
			}
