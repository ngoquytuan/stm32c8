...
#define SOCK_TCPS_SERVER_PORT 9095
////////////////////////////////////////////////
...
uint8_t serverIP[4] = {192, 168, 100,8};  //= {192, 168, 100,8}; 
uint8_t flag =0;
uint8_t counter = 0;
uint8_t send_tsp_flag = 0;
char data_to_send[30];//26
...
//=================================END_DHCP===============================//
  sprintf(data_to_send,"%s%.2f%s","#DCEFEF99AE4C\n#T1#",22.2,"\n##\r\n");
  while (1)
  {
		dhcp_rutine();
  }
...
if (debag == SOCK_TCPS){//создаем сокет
   ...
   if (debag == SOCK_OK){//если соединение установлено
		     ...
		     debag = send(SOCK_TCPS,buf,5);//sizeof(send_buf)
			 printf("%d:send\r\n",debag); 
