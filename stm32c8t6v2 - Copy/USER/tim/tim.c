...
void TIM_init(void)
{
	...
	TIMx_Init.TIM_Prescaler = 72;    // 72.000.000/7200=10.000
	TIMx_Init.TIM_CounterMode = TIM_CounterMode_Up;
	TIMx_Init.TIM_Period = 1000;// count 10000 tick then interrupt, gia tri max cua bo dem
	TIMx_Init.TIM_ClockDivision = 0;
	TIMx_Init.TIM_RepetitionCounter = 0;		//
	
	TIM_TimeBaseInit(TIM3,&TIMx_Init);	
	
	
	/* TIM IT enable */
  TIM_ITConfig(TIM3,TIM_IT_Update,ENABLE);									//1 		TIM_IT_Update
...
void NVIC_init(void)
{
	...
	NVIC_InitStructure.NVIC_IRQChannel = TIM3_IRQn;						//2  		 TIM3_IRQn
